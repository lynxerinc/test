...
def button(update, context):
    ...
    if isinstance(next_data, dict):
        ...
        if "€" in keys[-1]:  # This is a product with a price
            ...
            query.answer(f"Le produit {keys[-1]} a bien été ajouté au panier. Valeur totale : {sum([details['price'] * details['quantity'] for details in user_carts.get(update.effective_user.id, {}).values()])}€")
        else:
